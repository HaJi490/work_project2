...
export default function page() {
  ...
  const fetchTimeslotsForEdit = useCallback(async (reservation: Reservation) => { //📍
    ...
    try {
      ...
      const initialSelection = reservation.slot
        .map(slot => ({
          timeId: slot.timeId,
  ...
  const handleOpenEditPanel = (reservation: Reservation) => {
    ...
    setSelectedDate(reservationDate); // 패널에 표시할 날짜 설정
    fetchTimeslotsForEdit(reservation); // 해당 예약의 타임슬롯 정보 로드
  }

  // 4-3. 캘린더에서 날짜 변경시
  const handleDateChange = (date: Date) => {
    if (!reservationToEdit) return;
    // 만약 다른 날짜의 타임슬롯을 보여주고 싶다면, 여기서 fetchTimeslotsForEdit를 다시 호출
    // 날짜 선택기능만 담당
